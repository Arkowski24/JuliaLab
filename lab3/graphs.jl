...
function convert_to_graph(A ::Matrix{Int8}, nodes ::Vector{NodeType}, graph ::Vector{GraphVertex})
  #N = length(nodes)
...
function bfs(visited, remaining)
...
node_val_to_string(n ::Person) = "Person: $(n.name)\n"
node_val_to_string(n ::Address) = "Street nr: $(n.streetNumber)\n"

#= Returns text representation of the graph consisiting of each node's value
   text and number of its neighbors. =#
function graph_to_str(graph ::Vector{GraphVertex})
  #graph_str = ""
  graph_str = IOBuffer()
  for v in graph
    #graph_str *= "****\n"
    ...
    #end
    node_str = node_val_to_string(v.value)

    #graph_str *= node_str
    #graph_str *= "Neighbors: $(length(v.neighbors))\n"
    print(graph_str, node_str)
    print(graph_str, "Neighbors: $(length(v.neighbors))\n")
  end
  #graph_str
  String(take!(graph_str))
end
